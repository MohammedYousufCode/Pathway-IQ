...
Resend API key (resend.com — free, 100 emails/day)
---
1. Clone & Install
```bash
...
Click New bucket
Name: `resumes` | Set to Private
Go to Policies → add policy: allow authenticated users to `INSERT` and `SELECT` their own files
...
npm run dev
```
Open http://localhost:5173 — use Safari for full roadmap support while the cross-browser fix is pending.
---
🌍 Deployment (Vercel)
```bash
...
Click Deploy — Vercel auto-detects Vite and sets the correct build command
...
Groq	console.groq.com	Free — very generous rate limits
Resend	resend.com	Free — 100 emails/day
Supabase	supabase.com	Free — 500MB DB, 1GB storage
---
🐛 Troubleshooting
Problem	Cause	Fix
...
Verify DNS records
Update the `from` field in `supabase/functions/send-result-email/index.ts`
---
🛣 Roadmap
Quarter	Feature
